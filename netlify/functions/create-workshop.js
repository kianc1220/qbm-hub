...
function buildDashboardHtml({ name, date, time, venue, contact, slug, sheetUrl, siteUrl }) {
  const sheetIdMatch = sheetUrl && sheetUrl.match(/\/spreadsheets\/d\/([a-zA-Z0-9_-]+)/);
  ...
  const sheetBtn = sheetUrl
    ...
    : '';
  const sheetLinkInHeader = sheetUrl
    ? ` — Live data from <a href="${sheetUrl}" target="_blank" style="color:#1565c0;">Google Sheets</a>`
    : '';
  const errorSheetLink = sheetUrl
    ? `<a href="${sheetUrl}" target="_blank" style="color:#0f3460;font-weight:700;margin-left:8px;">Open Sheet →</a>`
    : '';
  return `<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .tag.some { background: #e3f2fd; color: #1565c0; }
    .tag.inter { background: #fff3e0; color: #e65100; }
    .refresh-note { font-size: 11px; color: #aaa; text-align: right; margin-bottom: 8px; }
    .data-error { background: #fff8ec; border: 1px solid #fde8a0; border-radius: 8px; padding: 14px 18px; margin-top: 12px; font-size: 13px; color: #7a5a00; display: none; }
...
<div class="container">
  ...
  <div class="cards">
    <div class="card gold"><div class="num" id="total">…</div><div class="lbl">Total Registrants</div></div>
    <div class="card"><div class="num" id="beginners">…</div><div class="lbl">Complete Beginners</div></div>
    <div class="card"><div class="num" id="someExp">…</div><div class="lbl">Some Experience</div></div>
    <div class="card"><div class="num" id="intermediate">…</div><div class="lbl">Intermediate</div></div>
  </div>
  ...
  <div class="table-box">
    <h3>Registered Attendees${sheetLinkInHeader}</h3>
    <table>
      <thead>
        <tr><th>#</th><th>Full Name</th><th>Phone</th><th>Email</th><th>Age</th><th>Experience</th><th>Heard From</th></tr>
      </thead>
      <tbody id="attendeeTable">
        <tr><td colspan="7" style="text-align:center;color:#aaa;font-style:italic;padding:24px;">Loading…</td></tr>
      </tbody>
    </table>
  </div>

  <div class="data-error" id="dataError">
    ⚠️ Could not load live data. Make sure the Google Sheet is shared as <strong>"Anyone with the link – Viewer"</strong>, then refresh.
    ${errorSheetLink}
  </div>
...
<script>
  const navy = '#1a1a2e', gold = '#e6a014', blue = '#457b9d', red = '#e94560', dark = '#16213e';
  const SHEET_ID = '${sheetId}';

  const expChart = new Chart(document.getElementById('expChart'), {
    type: 'doughnut',
    data: { labels: ['Beginner','Some Exp','Intermediate','Advanced'], datasets: [{ data: [0,0,0,0], backgroundColor: [navy,gold,blue,red], borderWidth: 2 }] },
    options: { plugins: { legend: { position: 'bottom', labels: { font: { size: 11 } } } } }
  });
  const heardChart = new Chart(document.getElementById('heardChart'), {
    type: 'bar',
    data: { labels: ['WhatsApp','Staff','Social','Friend','Walk-In','Other'], datasets: [{ data: [0,0,0,0,0,0], backgroundColor: [navy,gold,blue,red,dark,'#aaa'], borderRadius: 6 }] },
    options: { plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } }
  });
  const ageChart = new Chart(document.getElementById('ageChart'), {
    type: 'pie',
    data: { labels: ['36+','26-35','18-25','Under 18'], datasets: [{ data: [0,0,0,0], backgroundColor: [navy,gold,blue,red], borderWidth: 2 }] },
    options: { plugins: { legend: { position: 'bottom', labels: { font: { size: 11 } } } } }
  });
  ...
  function expTag(exp) {
    ...
    if (e.includes('beginner')) return '<span class="tag beginner">Beginner</span>';
    if (e.includes('some')) return '<span class="tag some">Some Exp</span>';
    if (e.includes('intermediate')) return '<span class="tag inter">Intermediate</span>';
    return '<span class="tag">' + esc(exp) + '</span>';
  }
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      if (resp.status !== 'ok') throw new Error(resp.errors?.[0]?.message || 'Sheet error');

      const { cols, rows } = resp.table;
      document.getElementById('dataError').style.display = 'none';

      if (!rows || rows.length === 0) {
        ['total','beginners','someExp','intermediate'].forEach(id => document.getElementById(id).textContent = '0');
        document.getElementById('attendeeTable').innerHTML = '<tr><td colspan="7" style="text-align:center;color:#aaa;font-style:italic;padding:24px;">No registrations yet.</td></tr>';
        updateNote(); return;
      }

      const headers = cols.map(c => (c.label || c.id || '').toLowerCase());
      const col = (k1, k2) => { const i = headers.findIndex(h => h.includes(k1)); return i >= 0 ? i : (k2 ? headers.findIndex(h => h.includes(k2)) : -1); };
      const nameCol=col('name'), phoneCol=col('phone'), emailCol=col('email');
      const ageCol=col('age'), expCol=col('experience'), heardCol=col('heard','where');
      const get = (row, idx) => (idx >= 0 && row.c[idx]) ? (row.c[idx].v ?? '') : '';

      const attendees = rows.map(row => ({
        name: get(row,nameCol), phone: get(row,phoneCol), email: get(row,emailCol),
        age: String(get(row,ageCol)), experience: String(get(row,expCol)), heard: String(get(row,heardCol))
      }));

      let beginner=0,some=0,intermediate=0,advanced=0;
      let age36=0,age26=0,age18=0,ageU=0;
      let hWA=0,hStaff=0,hSocial=0,hFriend=0,hWalk=0,hOther=0;
      attendees.forEach(a => {
        const exp=a.experience.toLowerCase();
        if(exp.includes('beginner')) beginner++; else if(exp.includes('some')) some++;
        else if(exp.includes('intermediate')) intermediate++; else if(exp.includes('advanced')) advanced++;
        const age=a.age.toLowerCase();
        if(age.includes('36')||age.includes('above')) age36++; else if(age.includes('26')) age26++;
        else if(age.includes('18')&&(age.includes('25')||age.includes('-25'))) age18++;
        else if(age.includes('under')) ageU++;
        const heard=a.heard.toLowerCase();
        if(heard.includes('whatsapp')) hWA++; else if(heard.includes('staff')) hStaff++;
        else if(heard.includes('social')) hSocial++; else if(heard.includes('friend')||heard.includes('family')) hFriend++;
        else if(heard.includes('walk')||heard.includes('poster')) hWalk++; else hOther++;
      });

      document.getElementById('total').textContent = attendees.length;
      document.getElementById('beginners').textContent = beginner;
      document.getElementById('someExp').textContent = some;
      document.getElementById('intermediate').textContent = intermediate;

      expChart.data.datasets[0].data = [beginner,some,intermediate,advanced]; expChart.update();
      heardChart.data.datasets[0].data = [hWA,hStaff,hSocial,hFriend,hWalk,hOther]; heardChart.update();
      ageChart.data.datasets[0].data = [age36,age26,age18,ageU]; ageChart.update();

      document.getElementById('attendeeTable').innerHTML = attendees.map((a,i) =>
        '<tr><td>'+(i+1)+'</td><td><strong>'+esc(a.name)+'</strong></td><td>'+esc(a.phone)+'</td><td style="font-size:12px;">'+esc(a.email)+'</td><td>'+esc(a.age)+'</td><td>'+expTag(a.experience)+'</td><td>'+esc(a.heard)+'</td></tr>'
      ).join('');
      updateNote();
    } catch(e) {
      console.error('Dashboard load error:', e);
      document.getElementById('dataError').style.display = 'block';
      ['total','beginners','someExp','intermediate'].forEach(id => document.getElementById(id).textContent = '?');
      document.getElementById('attendeeTable').innerHTML = '<tr><td colspan="7" style="text-align:center;color:#c62828;padding:24px;">Failed to load — see message below.</td></tr>';
    }
  }

  function updateNote() {
    const t = new Date().toLocaleTimeString('en-MY',{hour:'2-digit',minute:'2-digit'});
    document.getElementById('refreshNote').textContent = 'Last updated: ' + t + ' · Auto-refreshes every 60s';
  }
